...
local remoteEvent = Instance.new("UnreliableRemoteEvent",Storage)
...
function remoteHandler(data)
    ...
    if data.remote:IsA("UnreliableRemoteEvent") and lower(data.method) == "fireserver" then
...
local newindex = function(method,originalfunction,...)
    if typeof(...) == 'Instance' then
        ...
        if remote:IsA("UnreliableRemoteEvent") or remote:IsA("RemoteFunction") then
...
local newnamecall = newcclosure(function(...)
    ...
    if method and (method == "FireServer" or method == "fireServer" or method == "InvokeServer" or method == "invokeServer") then
        if typeof(...) == 'Instance' then
            ...
            if IsA(remote,"UnreliableRemoteEvent") or IsA(remote,"RemoteFunction") then    
...
function toggleSpy()
    if not toggle then
        ...
        if synv3 then
            ...
            originalEvent = hook(Instance.new("UnreliableRemoteEvent").FireServer, clonefunction(newFireServer))
            ...
            originalEvent = hookfunction(Instance.new("UnreliableRemoteEvent").FireServer, clonefunction(newFireServer))
